<?php
     /****** Query Get Users category Form Database  ****** */
    ...
    if(isset($_POST['add_city'])){
        ...
        if(empty($name_en) && empty($name_ar) && empty($desc_ar) && empty($desc_en) && empty($image['name']) && $cat == '..'){
        ...
        if(empty($msg)){
            ...
            $_SESSION['msg'] = 'City Add Successfully';
...
<!-- Start Content Add City  -->
    <div class="row pb-2">
        <form class="bg-dark p-3 rounded" action="?page=cities&add" method="POST" enctype="multipart/form-data">
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="mb-3 col-lg-6">
                    <label for="inputcountry" class="form-label  text-white">Category</label>
            ...
            <button type="submit" class="btn btn-primary" name="add_city">Add City</button>
        </form>
    </div>
    <div class="mt-5">
        <a href="?page=cities" class="btn btn-info"><i class="fa fa-backward" aria-hidden="true"></i> Back</a>
    </div>
<!-- End Content Add City  -->